...
async def cp(
    ...
    """Copy files or directories recursively with verification.

    Supports size verification and overwrite protection.
...
async def mv(
    ...
    """Move or rename files or directories with fault tolerance.

    Destination is verified before deleting origin. If any error occurs
    during move, the original source file remains preserved intact.
    ...
    try:
        ...
        if source.is_dir():
            if (
                ...
                raise ValueError(
                    f"Cannot move directory '{source}' into itself "
                    f"or its subdirectory '{target_dest}'."
                )
            ...
            def _verify_dir() -> None:
                if not target_dest.exists() or not target_dest.is_dir():
                    raise OSError(
                        f"Move verification failed: directory '{target_dest}' "
                        f"was not created properly"
                    )
            ...
            if source.is_symlink():
                ...
                def _move_symlink() -> None:
                    ...
                    if not target_dest.is_symlink():
                        raise OSError("Failed to create destination symlink")
                    source.unlink()
                ...
                try:
                    ...
                    def _verify_and_finalize() -> None:
                        ...
                        if src_size != dst_size:
                            raise OSError(
                                f"Move verification failed: size mismatch "
                                f"(source: {src_size}, dest: {dst_size})"
                            )
                        os.replace(tmp_target, target_dest)
                        if not target_dest.exists():
                            raise OSError("Failed to finalize destination file")
                        os.remove(source)
...
async def chmod(path: str, mode: int | str) -> list[TextContent]:
    """Change file or directory permissions with automatic rollback on failure.

    If the operation fails, permissions stay intact or rollback to the
    original state. The response JSON includes 'previous_mode' to enable
    immediate 1-step undo.
...
async def chown(
    ...
    """Change file or directory ownership with automatic rollback on failure.

    If the operation fails, ownership stays intact or rolls back to
    previous user and group. The response JSON includes 'previous_user'
    and 'previous_group' to enable immediate 1-step undo.
